...
├── Easy_Attention/        # 注意力与组件（11题：MHA/GQA_MLA/RoPE/RMSNorm/MLP/FFN/LoRA/SelfAttention/Activation/FlashAttn/SparseAttn）
├── Easy_Generator/        # 解码采样（5题：Greedy/TopK/TopP/Temperature/BeamSearch）
...
├── Easy_RL/               # 强化学习对齐（8题：REINFORCE/PPO/GRPO/GSPO/DPO/SimPO/RewardModel/RLHF_Pipeline）
...
### 📌 Easy_Attention — 注意力与组件（11 题）
激活函数、自注意力（基础版）、MHA(含 KV Cache，工程版)、GQA & MLA、RoPE、RMSNorm、MLP(SwiGLU)、FFN(PreNorm+GLU)、LoRA、FlashAttention(IO-aware tiling)、SparseAttention(滑动窗口)。每个文件末尾含测试验证 cell。

### 📌 Easy_Generator — 解码采样（5 题）
贪心搜索、Top-K、Top-P(nucleus)、温度采样、束搜索(BeamSearch)，含组合顺序说明。每个文件末尾含测试验证 cell。
...
### 📌 Easy_RL — 对齐算法（8 题）
REINFORCE（基础策略梯度）、PPO（RLHF 经典）、GRPO（组相对，省 critic）、GSPO（序列级重要性，Qwen3）、DPO（绕过 RM）、SimPO（去 reference）、RewardModel(Bradley-Terry)、RLHF_Pipeline(SFT→RM→PPO 三阶段)，均含题目背景 + 原理 + 考察点 + 测试验证。
