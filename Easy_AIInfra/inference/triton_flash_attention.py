...
def flash_attention_reference(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, causal: bool = False) -> torch.Tensor:
    """标准 attention 作为参考"""
    d_head = q.size(-1)
...
def flash_attention_tiled(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, block_size: int = 64, causal: bool = False) -> torch.Tensor:
    """
